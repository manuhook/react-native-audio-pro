...
const internalStore = exports.internalStore = (0, _zustand.create)((set, get) => ({
  ...
  updateFromEvent: event => {
    // Ce signal sert à la synchronisation ; il ne redéfinit pas les contrôles.
    if (event.type === _values.AudioProEventType.PLAYBACK_ACTIVITY_CHANGED) return;
    ...
    if (track) {
      ...
      const eventSignalsTrackSwap = type === _values.AudioProEventType.TRACK_TRANSITIONED || type === _values.AudioProEventType.STATE_CHANGED && (payload?.state === _values.AudioProState.LOADING || payload?.state === _values.AudioProState.PLAYING);

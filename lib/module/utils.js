...
export function validateTrack(track) {
  ...
  // Une plage invalide ne doit jamais être jouée comme le média entier.
  if (track.startMs !== undefined && (!Number.isSafeInteger(track.startMs) || track.startMs < 0) || track.endMs !== undefined && (!Number.isSafeInteger(track.endMs) || track.endMs <= (track.startMs ?? 0))) {
    logDebug('Track validation failed: invalid playback range');
    return false;
  }
 ...
 */
export function guardTrackPlaying(methodName) {

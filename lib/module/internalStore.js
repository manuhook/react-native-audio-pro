...
export const internalStore = create((set, get) => ({
  ...
  updateFromEvent: event => {
    ...
    if (event.type === AudioProEventType.PLAYBACK_ACTIVITY_CHANGED) return;
    ...
    if (track) {
      ...
      const eventSignalsTrackSwap = type === AudioProEventType.TRACK_TRANSITIONED || type === AudioProEventType.STATE_CHANGED && (payload?.state === AudioProState.LOADING || payload?.state === AudioProState.PLAYING);
